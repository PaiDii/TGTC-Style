...
def json_read_camera_parameters2(path, printout=False):
    ...
    if printout:
        parameters = [timeStamp, cameraEulerAngle, imageResolution, cameraTransform, cameraPos, cameraIntrinsics,
                      cameraView, cameraProjection]
        names = ['timeStamp', 'cameraEulerAngle', 'imageResolution', 'cameraTransform', 'cameraPos', 'cameraIntrinsics',
                 'cameraView', 'cameraProjection']
...
def alpha_composition(pts_rgb, pts_sigma, t_values, sigma_noise_std=0., white_bkgd=False):
    ...
    """
    sigma2alpha = lambda raw, dists, act_fn=torch.relu: 1. - torch.exp(-act_fn(raw) * dists)

    delta = t_values[..., 1:] - t_values[..., :-1]

    delta = torch.cat([delta, torch.tensor([[1e10]], device=delta.device).expand(delta[..., :1].shape)], -1)
    # delta = torch.cat([delta, torch.tensor([[1e10]], device=delta.device).expand(2048, 1)], -1)

    noise = 0.
    if sigma_noise_std > 0:
        noise = torch.randn(pts_sigma.shape, device=pts_sigma.device) * sigma_noise_std

    alpha = sigma2alpha(torch.relu(pts_sigma + noise), delta)

    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device=alpha.device), 1. - alpha + 1e-10], -1), -1)[:, :-1]

    rgb_exp = torch.sum(weights[..., None] * pts_rgb, -2)
...
def alpha_composition_wild(pts_rgb, pts_sigma, t_values, pts_transient_rgb, pts_transient_sigma, pts_transient_beta,
                           beta_min=0.03, sigma_noise_std=0., white_bkgd=False):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        pts_rgb: [num_rays, num_samples along ray, 3]. Prediction from model.
        pts_sigma: [num_rays, num_samples along ray]. Prediction from model.
    ...
    delta = t_values[..., 1:] - t_values[..., :-1]
    delta = torch.cat([delta, torch.Tensor([1e10]).expand(delta[..., :1].shape).to(pts_rgb.device)], -1)
    ...
    beta_exp = torch.sum(T_transient[..., None] * alpha_transient[..., None] * torch.relu(pts_transient_beta),
                         -2) + beta_min

    sigma_both = sigma_static + sigma_transient
    alpha_both = sigma2alpha(sigma_both, delta)
    T_both = torch.cumprod(torch.cat([torch.ones((alpha_both.shape[0], 1), device=alpha_transient.device), 1. - alpha_both + 1e-10], -1), -1)[:, :-1]

    rgb_exp = torch.sum(T_both[..., None] * alpha_static[..., None] * pts_rgb +
                        T_both[..., None] * alpha_transient[..., None] * pts_transient_rgb, -2)
...
L2_norm = lambda x: torch.sqrt(torch.sum(x ** 2) + 1e-8)
...
def get_rays(H, W, focal, cps, cx=None, cy=None, chunk=256):
    ...
    start = 0
    rays_o_total, rays_d_total = np.zeros([cps.shape[0], H, W, 3], np.float32), np.zeros([cps.shape[0], H, W, 3],
                                                                                         np.float32)
...
def sampling_pts_uniform(rays_o, rays_d, N_samples=64, near=0., far=1.05, harmony=False, perturb=False):
    ...
    ts = torch.linspace(0, 1, N_samples, device=rays_o.device).unsqueeze(0).expand(ray_num, N_samples)
    if not harmony:
        ts = ts * (far - near) + near
    else:
    ...
    if perturb:
        rand = torch.zeros([ray_num, N_samples], device=rays_o.device)
        torch.nn.init.uniform_(rand, 0, 1)
        mid = (ts[..., 1:] + ts[..., :-1]) / 2
        upper = torch.cat([mid, ts[..., -1:]], -1)
        lower = torch.cat([ts[..., :1], mid], -1)
        ts = lower + (upper - lower) * rand

    rays_o, rays_d = rays_o.unsqueeze(1).expand([ray_num, N_samples, 3]), rays_d.unsqueeze(1).expand(
        [ray_num, N_samples, 3])
...
def sampling_pts_fine(rays_o, rays_d, ts, weights, N_samples_fine=64):
    ...
    u = torch.rand(list(cdf.shape[:-1]) + [N_samples_fine], device=ts.device) * (1 - 1e-3)
    ...
    ts_near = torch.gather(ts_mid.unsqueeze(1).expand([ray_num, N_samples_fine, N_samples - 1]), -1,
                           below.unsqueeze(-1)).squeeze(-1)
    ts_far = torch.gather(ts_mid.unsqueeze(1).expand([ray_num, N_samples_fine, N_samples - 1]), -1,
                          above.unsqueeze(-1)).squeeze(-1)
    ...
    rays_o, rays_d = rays_o.unsqueeze(1).expand([ray_num, N_samples + N_samples_fine, 3]), rays_d.unsqueeze(1).expand(
        [ray_num, N_samples + N_samples_fine, 3])
...
def sampling_pts_fine_torch(rays_o, rays_d, ts, weights, N_samples_fine=64):
    ts_mid = 0.5 * (ts[..., 1:] + ts[..., :-1])
    t_samples = sample_pdf(ts_mid, weights[..., 1:-1], N_samples_fine, det=True)
    t_samples = t_samples.detach()
    t_vals = torch.sort(torch.cat([ts, t_samples], -1), -1)[0]
