...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, patch_size=8, in_chans=3, embed_dim=512):
        super().__init__()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos  # content+cape
...
class StyTrans(nn.Module):
    ...
    def forward(self, samples_c, samples_s):
        ...
        """
        if samples_c.shape[-1] == samples_c.shape[-2]:
            ...
            loss_c = self.calc_content_loss(self.normal(Ics_feats[-1]), self.normal(content_feats[-1])) + self.calc_content_loss(
                self.normal(Ics_feats[-2]), self.normal(content_feats[-2]))
            # Style loss
