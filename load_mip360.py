...
def spherify_poses(poses, bds, depths=0):
...
def load_mip_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    if spherify:
        poses, radius, bds, _ = spherify_poses(poses, bds)  # [B, 3, 5]
        poses = rerotate_poses(poses)  # [B, 3, 5]

        ### generate spiral poses for rendering fly-through movie
        centroid = poses[:, :3, 3].mean(0)
        radcircle = 0.9 * np.linalg.norm(poses[:, :3, 3] - centroid, axis=-1).mean()
        centroid[0] += 0.
        centroid[1] += 0.
        centroid[2] += 0.
        new_up_rad = -40 * np.pi / 180
