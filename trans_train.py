...
def train_transformer(
    ...
    embedding = StyTR.PatchEmbed(hidden_dim=hidden_dim)
    Trans = transformer.Transformer(d_model=hidden_dim, pos_embed_type=position_embedding)
    ...
    for i in tqdm(range(step, max_iter)):
        if i < 1e4:
            warmup_learning_rate(optimizer, iteration_count=i, lr=lr)
        else:
            adjust_learning_rate(optimizer, iteration_count=i, lr=lr, lr_decay=lr_decay)
