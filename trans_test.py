...
def transformer_render(
    ...
    if content:
        ...
        content_paths = [f for f in sorted(content_dir_path.glob('*')) if 'depth' not in str(f) and 'geometry' not in str(f)]
    ...
    for content_path in content_paths:
        for style_path in style_paths:
            ...
            cnt += 1
            output_name = '{:s}/{:03d}{:s}'.format(
                output_path, cnt, save_ext
            )
