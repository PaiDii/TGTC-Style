...
def train_temporal_invoke(save_dir, sv_name, log_dir, is_ndc, nerf_content_dir, style_dir, batch_size, data_p, n_threads=8, lr=5e-4, max_iter=100):  # max_iter=1000
    trans_train.train_transformer(nerf_content_dir, style_dir)
    ...
    decoder.load_state_dict(torch.load('./models/decoder.pth'))
    shutil.copy('./models/decoder.pth', save_dir + '/' + sv_name)
    step = 0
    vgg.load_state_dict(torch.load('./pretrained/vgg_normalised.pth'))
    vgg = nn.Sequential(*list(vgg.children())[:31])
    ...
    if not os.path.exists(data_p):
        os.makedirs(data_p)
    trans_test.transformer_render(nerf_content_dir, style_dir, data_p, decoder_path=save_dir + '/' + sv_name)
    writer.close()
